<template>
  <v-app id="keep">
    ...
    <v-content>  
      <v-layout row-wrap>
        <v-flex xs12 md7 sm7 d-flex>
          
      <li v-for="(active, menu) in menus" :key="menu.active"
        :class="{
          'nav__label--active' : active,
          'nav__label--filter': activeFilters[menu].length
        }" @click="setMenu(menu, active)" style="list-style:none;">
        <v-chip outline label color="red">
          {{ menu }}
        </v-chip>
  ...
  <transition-group name="dropdown" tag="section" class="dropdown" :style="dropdown">
    <menu v-for="(options, filter) in filters" class="filters"
      ...
      <template v-else>
        <li v-for="(active, option) in options" class="filters__item" :key="option.active"
    ...
    methods: {
     ...
     data: () => ({
      ...
      items: [
        ...
        { icon: 'help', text: 'Help' , link: 'label'},
